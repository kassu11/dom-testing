...
function hasErrors(commands, path, baseCommand) {
    ...
    const lastValidKey = (path.findLast(p => p !== null)?.next || "");
    const customError = baseCommand.commands[lastValidKey]?.error?.(commands[errorIndex] ?? "") ?? "";
    if (customError && (baseCommand.commands[lastValidKey]?.type !== "optional" || commands[errorIndex]?.length))
        return customError;
    ...
    if (baseCommand.commands[lastValidKey]?.type === "required") {
        if (commands[errorIndex]?.length)
            return `Expected ${baseCommand.commands[lastValidKey].help} but got "${commands[errorIndex]}".`;
        return `Part of the command is missing, continue by adding ${baseCommand.commands[lastValidKey].help}`;
    }
    if (errorIndex !== -1 && commands[errorIndex].length === 0)
        return `Remove empty space!`;
    if (errorIndex !== -1)
