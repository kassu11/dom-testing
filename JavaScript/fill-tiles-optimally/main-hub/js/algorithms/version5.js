...
function megaMergeTiles(bigTiles) {
	...
	function getNierestTiles(tile, smallMode = false, rootObject) {
		const returnObject = {
			minX: tile.x,
			maxX: tile.x + tile.wx,
			minY: tile.y,
		...
		returnObject.tileSet.forEach(nTile => {
			...
			const nearestData = getNierestTiles(nTile, true, rootObject || returnObject);
